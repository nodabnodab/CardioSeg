...
def train_pipeline():
    ...
    weight_decay = 1e-5
    roi_size = (128, 128, 8) 
    early_stopping_patience = 8  # Stop if no improvement after 8 validation evaluations (16 epochs)
    ...
    try:
        for epoch in range(1, max_epochs + 1):
            ...
            if epoch % val_interval == 0:
                ...
                # Carry over last known validation scores for visualization alignment
            ...
            torch.save(checkpoint, os.path.join(base_dir, "latest_checkpoint.pth"))
        ...
        # Guarantee saving files and generating plot even on crash/exit
        if history["epoch"]:
            with open(history_file, "w") as f:
                json.dump(history, f, indent=4)
            save_training_plot(history, plot_file)
            print("Successfully saved training history JSON and generated training curves chart.")
